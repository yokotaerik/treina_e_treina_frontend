  ...
  return (
    <div className="bg-white p-8 flex flex-col gap-2">
      ...
      <p className="text-lg font-semibold ">
        {workout.worksheet.description}
      </p>
      <p className="text-3xl font-bold text-orange-500">Criado em:</p>
      ...
      {workout.exercises.map((exercise, index) => (
        <div key={exercise.id} className="mb-8 flex flex-col gap-1">
          <h3 className="text-2xl text-gray-700 font-bold ">{exercise.info.name}</h3>
          <p className="text-xl font-semibold ">Detalhes do exercício:</p>
          <p className="text-lg text-gray-800 font-semibold">{workout.worksheet.exercises[index]?.notes}</p>
          <ul className="my-2">
            {exercise.sets.map((set, index) => (
              <li key={set.id} className="flex flex-col mb-4">
                <p className="font-bold text-gray-700 text-lg mb-2">{index + 1}º serie</p>
      ...
      <button
        ...
        className="px-4 py-2 bg-orange-500 text-white hover:bg-orange-600 w-full md:w-auto"
      >
        Enviar
      </button>
    </div>
